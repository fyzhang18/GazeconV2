...
raw.data1 =  read.csv("/Volumes/emberson/ResearchProjects/Pupillometry/GazeCon/Data/GazeConV2_1-6.csv")
raw.data2 =  read.csv("/Volumes/emberson/ResearchProjects/Pupillometry/GazeCon/Data/GazeConV2_7-12.csv") 
orig <- rbind(raw.data1,raw.data2)
...
#7. Fill in blinks (up to 85 samples)
...
#two subjects don't have baseline for trial 1

#9. Calculate percentage change from baseline
pooC <- poo
...
for (j in 1:length(subs)) {
  ...
  print(j)
  for (i in 1:length(trials)) {
    if (trials[i]==0){ #if it's trial index 0 do nothing
      print("skip trial 0")
    } else { #if it's trial index 1+ 
...
```{r}
write.csv(poo, "GC1-12_preprocessed_all.csv", row.names=TRUE) #save to computer
write.csv(poo4, "GC1-12_preprocessed_final.csv", row.names=TRUE) #save to computer
```
